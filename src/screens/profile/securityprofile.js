...
export default function SecurityProfile({ navigation }) {
  ...
  const [shareData, setShareData] = useState(userData.permissions[1]);

  async function update() {
    const updatedPermissionsArray = [...userData.permissions];
    ...
    >
      <ScrollView>
        ...
        >
          ...
          Política de Privacidade que deverá ser autorizada pelo utilizador.
          {"\n"}
          {"\n"}
        </Text>
        ...
        <View
          style={[
            dark_mode ? dark_styles.boxOptions : styles.boxOptions,
            {
              borderBottomWidth: 0,
              borderTopWidth: 1,
              borderTopColor: dark_mode ? CONST.greyColor : CONST.dividerColor,
            },
          ]}
        >
          <Text
